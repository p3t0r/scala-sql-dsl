...
<pre>
%{color:blue}scala>% val q = select ("*") from ("user") where (("name","peter") and (("active", true) or ("role", "admin")))
%{color:blue}scala>% q.sql
...
<pre>
%{color:blue}scala>% val sql = """select name,age from users where name = "peter" and (active = true or age = 30)"""
%{color:blue}scala>% val query = p.parse(sql).get
%{color:blue}scala>% AnsiSqlRenderer.sql(query)
